...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        if epoch_num > 190 and (epoch_num + 1) % save_interval == 0:
        # if epoch_num > 190 and (epoch_num)  % save_interval == 0:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            save_mode_path = os.path.join(snapshot_path, 'epoch_{}_{}.pth'.format(epoch_num, timestamp))
