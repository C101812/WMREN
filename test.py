...
from networks.WMREN import WMREN
...
parser.add_argument('--output_dir', type=str,
                    default='D:\LU\WMREN - 副本\model_out_20_400', help='output dir')
...
def inference(args, model, test_save_path=None):
    ...
    m = np.array(['主动脉', '胆囊', '左肾', '右肾',  '肝脏', '胰腺', '脾脏', '胃'])
    for i in range(1, args.num_classes):
        logging.info('Mean class %d mean_dice %f mean_hd95 %f 类别 %s' % (i, metric_list[i-1][0], metric_list[i-1][1],m[i-1]))
...
if __name__ == "__main__":
    ...
    net = WMREN(num_classes=args.num_classes).cuda(0)
    model_dir = args.output_dir  # Ensure this directory contains all .pth files

    # Log setup
    log_folder = './/test_log//test_log_'
    os.makedirs(log_folder, exist_ok=True)
    logging.basicConfig(filename=log_folder + '/all_models_log.txt', level=logging.INFO,
